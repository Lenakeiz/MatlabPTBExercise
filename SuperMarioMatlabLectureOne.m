...
% In the game, like in Mario Bros, we need to help Mario avoid different
% kind of enemies (presented as images on the screen). The game is divided
% in two parts, a tutorial and testing phase. During tutorial part we will
% show which key needs to be pressed upon an image presentation. During the
% testing phase enemies will be presented in a random order and the user
% will have a fixed amount of time to press the correct key. In this game
% we are interested to collect the information of which key the user has
% pressed and how long did it take to give a response.]

% [After lecture one we are going to make the training part as an exercise and
% in after lecture two are going to add the testing part.]
...
%% Closing
% [After exiting the previous function we have now completed the game. We
% can now safely close all of the opened PTB windows. For doing so we can
% call the Screen sub-function 'CloseAll' to closea all of them at once.
